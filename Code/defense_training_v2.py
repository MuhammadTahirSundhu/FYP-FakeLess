...
class ASVEmbedder:
    ...
    '''    
    def extract(self, wav_tensor_or_np):
        if isinstance(wav_tensor_or_np, np.ndarray):
            wav_tensor = torch.as_tensor(wav_tensor_or_np, dtype=torch.float32, device=self.device)
        else: # Assume it's a tensor
            wav_tensor = wav_tensor_or_np.float().to(self.device)

        emb = self.model.encode_batch(wav_tensor) 

    
        return emb.squeeze(0).to(self.device)
    '''
    '''
    ...
    '''
    # In defense_training.py, inside class ASVEmbedder:

    def extract(self, wav_tensor_or_np):
        """
        Accepts: 1D numpy array OR 1D torch tensor of audio samples (float32, 16kHz).
        Returns: 1D torch embedding tensor.
        CRITICAL: Removed torch.no_grad() to preserve gradient path to the input tensor.
        """
        if isinstance(wav_tensor_or_np, np.ndarray):
            # Orig waves (NumPy) will be converted to tensor. This path has no gradient history.
            wav_tensor = torch.as_tensor(wav_tensor_or_np, dtype=torch.float32, device=self.device)
        else: 
            # Protected waves (Tensor) will be used directly. The gradient history MUST be active here.
            wav_tensor = wav_tensor_or_np.float().to(self.device)

        # Convert to mono and add batch dimension [1, time]
        if wav_tensor.dim() == 2 and wav_tensor.shape[0] > 1:
            wav_tensor = wav_tensor.mean(dim=0)
        if wav_tensor.dim() == 1:
            wav_tensor = wav_tensor.unsqueeze(0)
        self.model.eval()
        # Extract embedding. The gradient will flow through here IF the input was a protected tensor.
        emb = self.model.encode_batch(wav_tensor)  # shape [1, emb_dim]

        # The detach() is not strictly needed for the protected wave, but doesn't hurt.
        return emb.squeeze(0).to(self.device)
...
config = {
    ...
    "save_dir": "checkpoints_phase2_1",
    "train_manifest": "Data/Librispeech_prepared/train-clean-100_small_manifest.txt",
        ...
        for paths, waves in pbar:
            ...
            if asv_embedder is not None:
                ...
                for b in range(mel_prot.size(0)):
                    mel_tensor_b = mel_prot[b] # [n_mels, T] - PROTECTED MEL TENSOR

                    # 1. Protected Wave: Use HiFi-GAN and keep as TENSOR
                    # mel_to_wave_hifigan returns [1, 1, T_wav]
                    wav_prot_tensor = mel_to_wave_hifigan(mel_tensor_b, hifigan_vocoder, SR)
                    
                    # 2. Original Wave: Load original NumPy wave and convert to TENSOR
                    orig_wav_np = waves[b] 
                    # We use the ASVEmbedder.extract() which handles the NumPy->Tensor conversion
                    e_orig = asv_embedder.extract(orig_wav_np).to(device)
                    
                    # 3. Extract protected embedding (pass the reconstructed WAV TENSOR)
                    # Squeeze to [T_wav] for the embedder
                    e_prot = asv_embedder.extract(wav_prot_tensor.squeeze(1)).to(device) 
                ...
                '''
                    ...
                    #wav_recon = mel_to_wave_hifigan(mel_t,hifigan_vocoder,SR)
                    ...
                    #e_prot = asv_embedder.extract(wav_recon.squeeze(0)).to(device)
                    attack_losses.append(cosine_similarity_torch(e_orig, e_prot))
                # we want to minimize similarity (i.e., decrease attack success). Convert to torch scalar.
                attack_loss = torch.stack(attack_losses).mean()
                '''
